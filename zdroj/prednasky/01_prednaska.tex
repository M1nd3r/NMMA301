...
x \\ y
\end{pmatrix}\text{.}$$
%Platí díky tomu, že $\mathbb{C} \cong \mathbb{R}^2$.
%Všimneme si, že každé $\mathbb{C}$\emph{-lineární zobrazení} je $\mathbb{R}$\emph{-lineární zobrazení}, ale matice tohoto lineárního zobrazení je speciální viz pozorování níže.
\end{definition}
...
%Jelikož máme dva typy lineárních zobrazení (podle toho zda se na tu rovinu díváme jako na $\mathbb{R}^2$, nebo jako na $\mathbb{C}$) tak dává smysl abychom měli i dva typy diferencovatelnosti.Jako první si uvedeme reálnou diferencovatelnost (funkci $f$ aproximujeme $\mathbb{R}$\emph{-lineárním zobrazením}).
\begin{definition}
...
%hodí se nezapomenout že doted jsme mluvili pouze o reálné diferencovatelnosti. Jako druhou si ted zavedeme komplexní diferencovatelnost(funkci $f$ aproximujeme $\mathbb{C}$\emph{-lineárním zobrazením}).
\end{note} 
...
%Tak i pro komplexní funkce jedné komplexní proměnné platí stejné věty jako pro reálné funkce jedné reálné proměnné. Tedy platí: $(f\pm g)'$, $(f.g)'$, $(f/g)'$ a $(f\circ g)'$.
...
\begin{enumerate}
    ...
    \item Existuje $\diff f(z_0)$ a $\diff f(z_0)$ je $ \mathbb{C}$-lineární %existuje TD v $z_{0}$ a ten TD je komplexně lineární
...
\begin{note}\mbox{}
\begin{itemize}
    \item Existuje-li $f'(z_0)$, potom $df(z_0)h = f'(z_0)h, h \in \mathbb{C}$ a $f'(z_0) = \frac{\partial f}{\partial x}(z_0)$ %Poslední rovnost platí co do hodnoty, nikoliv co do existence. Muže se stát, že všechny reálné parciální derivace existují, ale ta komplexní nikoliv....proto tam máme to existuje-li $f'(z_0)$, potom...
